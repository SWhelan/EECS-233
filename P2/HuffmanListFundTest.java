...
public class HuffmanListFundTest {
 ...
 public void testByteAndIntegerArrayConstructor() {
  HuffmanList list = new HuffmanList(new byte[]{(byte)'a',(byte)'b'},
                                     new int[]{1,2});
  assertEquals(list.getFirst().getByte(), (byte)'a');
  assertEquals(list.get(1).getByte(), (byte)'b');
  assertEquals(list.getFirst().getCount(), 1);
  assertEquals(list.get(1).getCount(), 2);
  
  HuffmanList list2 = new HuffmanList(new byte[]{(byte)'b',(byte)'a'},
                                       new int[]{2,1});
  assertEquals(list2.getFirst().getByte(), (byte)'a');
  assertEquals(list2.get(1).getByte(), (byte)'b');
  assertEquals(list2.getFirst().getCount(), 1);
  assertEquals(list2.get(1).getCount(), 2);
